...
export async function generatePersonalizedInsights() {
  ...
  if (prevTotalExpenses > 0 && totalExpenses > 0) {
    ...
    if (expenseChange > 20) {
      ...
      insights.push({
        ...
        message: `Great job! Your spending decreased by ${Math.abs(expenseChange).toFixed(0)}% compared to last month ($${prevTotalExpenses.toFixed(2)} → $${totalExpenses.toFixed(2)}).`,
        priority: 3
      });
    }
  }
  ...
  if (currentSavings > 0) {
    const monthsToGoal = Math.ceil(savingsGoal / currentSavings);
    insights.push({
      type: 'tip',
      category: 'Savings',
      title: 'Savings Progress',
      message: `At your current savings rate of $${currentSavings.toFixed(2)}/month, you'll reach your $${savingsGoal.toFixed(2)} goal in ${monthsToGoal} month${monthsToGoal > 1 ? 's' : ''}.`,
      priority: 3
    });
  }
