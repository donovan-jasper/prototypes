import { addMinutes, isWithinInterval, startOfDay, endOfDay, setHours, setMinutes } from 'date-fns';
import { Task, ScheduleBlock, TimeSlot, Routine } from '../types';
...
export function adaptRoutine(
  ...
  const windowStart = setMinutes(setHours(today, preferredStart), 0);
  const windowEnd = setMinutes(setHours(today, preferredEnd), 0);
  
  // Find available slots within the preferred window
  const availableSlots = findAvailableSlots(today, schedule);
