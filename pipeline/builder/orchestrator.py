...
def _update_root_readme():
    ...
    try:
        ...
        for dirname in sorted(os.listdir(PROTOTYPES_DIR)):
            ...
            feasibility = idea.get("feasibility_score") if idea else None

            if not app_name:
                app_name = (idea["title"] if idea else dirname)[:35]

            # Trim verbose prefixes and cap length
            desc = summary.strip()
            for prefix in [
                "A mobile app that ", "A mobile-first ", "An AI-powered ",
                "A comprehensive ", "A universal ", "A personalized ",
                "A privacy-focused ", "A hyper-local ", "A cross-platform ",
                "A voice-first, AI-powered ", "A customizable, ",
                "A social ", "A gamified, ", "A premium, ",
                "A habit-building ", "A sleep-focused ",
            ]:
                if desc.startswith(prefix):
                    desc = desc[len(prefix):]
                    desc = desc[0].upper() + desc[1:] if desc else desc
                    break
            # Cut at word boundary, keep it short
            if len(desc) > 60:
                desc = desc[:60].rsplit(" ", 1)[0]
            desc = desc.rstrip(" ,.")

            entries.append((dirname, app_name, desc, score, feasibility))
        ...
        for dirname, name, desc, score, feas in sorted(entries, key=lambda x: -(x[3] or 0)):
            v = str(score) if score else "-"
            f = str(feas) if feas else "-"
            lines.append(f"| [{name}]({dirname}/) | {desc or '--'} | {v} | {f} |")
