...
export class AVRCompiler {
  private wasmModule: WebAssembly.Module | null = null;
  private wasmInstance: WebAssembly.Instance | null = null;

  async initialize(): Promise<void> {
    try {
      // Load the WASM module for AVR compilation
      const response = await fetch('lib/compiler/wasm/avr-toolchain.wasm');
      ...
      console.error('Failed to initialize AVR compiler:', error);
      throw error;
    }
  }
  ...
  async compile(code: string): Promise<CompilationResult> {
    ...
    try {
      logs.push('[INFO] Compiling AVR code...');
      ...
      const result = exports.compile_avr(codePtr);
